...
| **OmniBar** | Tracking (alternativa) | Barra dedicada de CDs enemigos (kicks, stuns, defensivos), lee el combat log (legal en Midnight) | Solo si el formato de MiniCC no te convence para kicks — es redundante tenerlo junto a MiniCC | ⚪ Opcional |
...
1. **Una sola instalación:** CurseForge App → Skill Capped UI → marcar los 6 perfiles core (sArena, MiniCC, FrameSort, BBP, BBF, Details) → Install → `/reload`.
